...
- 对地址的冗余计算、检查和翻译
- 利用控制流来实现掩码操作，导致宿主机分支预测错误较为频繁

此外，我们通过以下方法，使得编译器（GCC11）能将代码自动向量化，从而利用 X86 的 AVX 指令进行 RVV 的模拟：
- 用位运算来实现掩码操作
- 避免代码中出现 unbounded 的循环
...
![Perf report of QEMU running h264](qemu-vector-hotspot2.png)
我们阅读代码发现，QEMU 的 MMU 实现并不高效，这可能与 QEMU 要考虑更多的指令集、访存模式、页面大小有关。
例如对 TLB 的访问的 common path 上需要进行多次链式访存：
![QEMU mmu](qemu-mmu-lookup.png)

![QEMU TLB entry](qemu-tlb_entry.png)

为此，余子濠博士在 NEMU 中实现了一个更高效的 MMU，从 guest 的虚地址到 host
的虚地址的转化的 common path 只需要执行如图所示的几行代码。

![NEMU translate](nemu-tlb.png)

# 动机

即使如此，NEMU 7-8 MIPS 的仿真速度也远远无法满足我们的软硬件迭代需求：
...
在元素粒度进行模拟导致了以下问题：
- 目前所有的实现都为每一个元素进行了权限检查和地址翻译，这在大部分情况下是不必要的
- 用控制指令实现掩码操作导致了难以预测的分支，从而产生了更多的分支预测错误
- 低位宽的操作没有充分利用宿主机的计算宽度（8 bit VS. 64 bit）


# 优化

针对上述问题，我们给出了以下优化思路：
- 优化1：对于数据在内存中连续的指令，在 TLB hit 且不跨页的情况下只进行一次地址翻译和权限检查
- 优化2：用 `and` 和 `or` 来手动实现掩码操作，避免引入数据依赖的控制流
- 优化3：尽可能使用 bounded loops，使得循环可以被编译器自动向量化
...
我们运行测试的机器是 7950x3d。
我们进行性能测试的对象是 SPECint 2006 中可以自动向量化的子项，包括 `hmmer` 和 `h264` 。
...
我们将 NEMU 分为 3 种版本：
- baseline：优化前的 NEMU
- vectorizable：优化后的 NEMU，但是没有开启自动向量化 `-ftree-vectorize`
- vectorized：优化后的 NEMU，开启了自动向量化

![h264 early mips](h264_early_mips.png)
...
![h264 early branch misses](branch-misses.png)
![vst branch misses](vst-continue-10pct.png)

作为对比，我们的优化后的代码中，`branch-misses` 降低到了约 330M：
![vectorized branch misses](vectorized-branch-misses.png)
